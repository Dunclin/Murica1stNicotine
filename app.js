/* Cart + map + quotes with distance fix */
...
function renderCart(){const cart=getCart();if(cartItemsEl) cartItemsEl.innerHTML='';let t=0;cart.forEach(i=>{t+=i.price*i.qty;if(cartItemsEl){const row=document.createElement('div');row.className='cart-row';row.innerHTML=`<div><strong>${i.name}</strong> <small>(${i.flavor})</small></div><div>x ${i.qty} — $${(i.price*i.qty).toFixed(2)}</div>`;cartItemsEl.appendChild(row);}});if(cartTotalEl) cartTotalEl.textContent=t.toFixed(2);if(cartCountEl) cartCountEl.textContent=cart.reduce((a,b)=>a+b.qty,0);if(checkoutLink) checkoutLink.classList.toggle('primary',getCart().length>0);} 
...
function chooseAddr({label,lat,lon}){addrInput.value=label;destLatLng={lat,lng:lon};L.marker([lat,lon]).addTo(lMap);drawRoute();acList.innerHTML='';acList.classList.add('hidden');quoteStatus && (quoteStatus.textContent='Click "Get delivery quote" to compute fees.');}
